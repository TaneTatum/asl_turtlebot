    ...
    def solve(self):
        while len(self.open_set) > 0:
            ...
            if x_current == self.x_goal:
            	...
            	return True
            self.open_set.remove(x_current)		# Alg line 10
            self.closed_set.append(x_current)	# Alg line 11
            for x_neigh in self.get_neighbors(x_current):
            	if x_neigh in self.closed_set:
            		continue
            	tent_g = self.g_score.get(x_current) + self.distance(x_current,x_neigh)	# Alg line 16
            	if x_neigh not in self.open_set:
            		self.open_set.append(x_neigh)
            	elif tent_g > self.g_score.get(x_neigh):					# Alg line 19
            		continue
